...
export function bsp_gen(map)
{
  const root = brush_collapse(new split_brush_t(map.brushes[0].faces, []));
  
  for (let i = 1; i < map.brushes.length; i++)
    brush_insert_R(root, new split_brush_t(map.brushes[i].faces, []));
...
function split_face_even(vbehind, vmiddle, vahead, plane, normal)
{
  ...
  const behind = new map_face_t([vbehind, shared, vmiddle], normal);
  const ahead = new map_face_t([vahead, shared, vmiddle], normal);
...
function split_face_uneven(vbase, vhead, plane, normal, flip)
{
  ...
  const head = [ new map_face_t([hit_a, hit_b, vhead], normal) ];
  
  const base = [
    new map_face_t([vbase[0], vbase[1], hit_b], normal),
    new map_face_t([hit_b, hit_a, vbase[0]], normal)
  ];
...
function split_brush(brush, plane)
{
  ...
  if (faces_behind.length > 0) {
    ...
    brush_behind = new split_brush_t(faces_behind, planes);
  }
  ...
  if (faces_ahead.length > 0) {
    ...
    brush_ahead = new split_brush_t(faces_ahead, planes);
  }
...
function brush_collapse(brush)
{
  ...
  if (planes.length == 1) {
    return new bsp_node_t(map_brush_type.BRUSH_SOLID, planes[0]);
  } else {
    const root = new bsp_node_t(map_brush_type.BRUSH_EMPTY, planes[0]);
    
    let node = root;
    for (let i = 1; i < planes.length - 1; i++) {
      node.behind = new bsp_node_t(map_brush_type.BRUSH_EMPTY, planes[i]);
      node = node.behind;
    }
    
    node.behind = new bsp_node_t(map_brush_type.BRUSH_SOLID, planes[planes.length - 1]);
